 ...
 * puede comprobar que el codigo llama a `.focus()`, no que el recorrido sea el
 * que se cree. Lo que se comprueba aqui es el recorrido de verdad.
 *
 * **Por que existe.** Nacio en la rama de la migracion a Tailwind, cubriendo el
 * fallo que aquella arreglo de paso; el revert de `6a5845f` devolvio el fallo y
 * se llevo la prueba. Se recupera con el arreglo, porque no depende del motor de
 * estilos: el ciclo del tabulador es comportamiento.
 *
 * El fallo era este: el idioma, el tema y la sesion viven en `.acciones`, FUERA
 * de `#menu-principal`, y en movil **se ven**. Con el ciclo limitado al panel, el
 * tabulador desde el ultimo enlace volvia al boton y no habia forma de llegar a
 * ellos sin cerrar el menu. Un control visible que el teclado no alcanza es
 * WCAG 2.1.1.
 */

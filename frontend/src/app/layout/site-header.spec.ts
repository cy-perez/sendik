...
describe('SiteHeader', () => {
  ...
  beforeEach(() => {
    ...
    // La clase es la marca del tema y la unica. Reiniciarla importa: sin esto la
    // prueba siguiente arrancaria en el estado que dejo la anterior.
  ...
  /**
   * La clase, ida y vuelta. Es lo unico que marca el tema desde que se retiro
   * tokens.css: el atributo data-tema que se escribia en paralelo existia solo
   * porque esa hoja lo leia, y se fue con ella.
   */
  it('el conmutador pone y quita la clase de modo oscuro', async () => {
    const fixture = await render();

    (byLabel('Cambiar a modo oscuro') as HTMLButtonElement).click();
    await fixture.whenStable();

    expect(document.documentElement.classList.contains('dark')).toBe(true);

    (byLabel('Cambiar a modo claro') as HTMLButtonElement).click();
    await fixture.whenStable();
  ...
  describe('foco atrapado en el menu compacto', () => {
    ...
    /*
     * Aqui vivian dos casos que recorrian el ciclo del tabulador: «desde el
     * ultimo enfocable vuelve al boton» y «con shift desde el boton va al
     * ultimo». Se fueron a e2e/foco-cabecera.spec.ts al pasar la trampa a
     * cdkTrapFocus (ADR-0032), y conviene saber por que, porque no es que
     * estorbaran.
     *
     * jsdom no implementa el tabulador: pulsar Tab no mueve el foco. Los dos
     * casos pasaban solo porque el manejador escrito a mano llamaba a .focus()
     * el mismo, asi que en realidad comprobaban esa llamada, no el recorrido.
     * El CDK trabaja al reves —pone anclas invisibles en los bordes de la region
     * y deja que el navegador les lleve el foco de verdad—, y eso jsdom no lo
     * puede ver: aqui la prueba daria siempre falso negativo.
     *
     * En Playwright el tabulador es el de Chromium, asi que alli se comprueba el
     * recorrido real, y de paso lo que estas dos NO veian: que el idioma, el
     * tema y la sesion tambien se alcanzan.
     */

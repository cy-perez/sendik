...
describe('SpinViewer', () => {
  ...
  /*
   * Aqui vivia `describe('el desplazamiento de la página')`, con el caso que
   * comprobaba `touch-action` (criterio 17). Se fue entero a
   * e2e-completo/captura-y-visor.spec.ts, y no por estorbar.
   *
   * Leia el estilo computado del marco, y eso funcionaba porque la regla vivia en
   * spin-viewer.css: Angular incrusta la hoja del componente tambien en la
   * prueba. Desde la ADR-0032 es una utilidad de la hoja GLOBAL, que jsdom no
   * carga, asi que aqui el estilo computado sale vacio y el caso daria siempre un
   * falso negativo. En Chromium la hoja si esta, y de paso se comprueba sobre la
   * ficha de verdad y no sobre un anfitrion de prueba.
   */

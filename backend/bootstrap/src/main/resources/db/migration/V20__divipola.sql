-- La division politico-administrativa de Colombia. HU-016, RN-100.
--
-- Es la lista cerrada de la que se elige el municipio de una direccion de
-- entrega. No se escribe a mano: el municipio es la entrada de la cotizacion
-- (RN-039) y la que decide si hay cobertura (RN-080), y escrito a mano
-- "Bogota", "bogota" y "Bogota D.C." son tres destinos distintos de los que
-- ninguno cotiza.
--
-- FUENTE. Portal de datos abiertos del Estado colombiano, los dos conjuntos
...
-- Version del conjunto: actualizado el 18 de mayo de 2026.
-- Descargado el 11 de septiembre de 2026.
--
-- LO QUE TRAE, y el criterio 22 exige que coincida con la fuente: 33
-- departamentos y 1122 municipios. El 33 incluye a Bogota D.C., que en la
-- Divipola es departamento y municipio a la vez; el 1122 incluye 18 areas no
-- municipalizadas -- Amazonas, Guainia y Vaupes tienen varias -- y 1 isla.
-- Los tres tipos viven en la misma tabla porque para una direccion de entrega
-- son lo mismo: el sitio al que hay que llevar algo. Dejar fuera 19 filas
-- porque no son "municipio" en sentido estricto seria sembrar algo distinto de
-- la fuente sin decirlo, y ademas dejar sin direccion a quien vive alli.
--
-- LOS NOMBRES llegan en mayuscula sostenida y aqui van capitalizados, que es
-- como se leen en pantalla. Son nombres propios y no se traducen: van iguales
-- en espanol y en ingles (criterio 26).
--
-- LOS CODIGOS SON JERARQUICOS y de eso depende una decision del dominio: los
-- dos primeros digitos del codigo de municipio son los del departamento, sin
-- una sola excepcion en las 1122 filas. Por eso el cuerpo de la API pide solo
-- el municipio: el departamento se deriva, y un par incoherente no puede
-- existir porque no hay par.
--
-- SE MARCA INACTIVO EN VEZ DE BORRARSE. Cuando el DANE suprima o renombre un
-- municipio, la fila se queda y `active` pasa a falso: borrarla rompe la
-- direccion guardada de alguien, y la clave foranea de V21 lo impide a
-- proposito con ON DELETE RESTRICT.
--
-- VIVE EN `shared` Y NO EN `identity`. La usan las direcciones hoy y el
-- cotizador de envios manana; no es de nadie en particular (ADR-0040).
...
COMMENT ON TABLE departments IS
    'Los 33 departamentos de la Divipola del DANE. Bogota D.C. esta aqui y tambien en municipalities';
...
COMMENT ON TABLE municipalities IS
    'Los 1122 municipios, areas no municipalizadas e islas de la Divipola. Lista cerrada de RN-100';

COMMENT ON COLUMN municipalities.active IS
    'Falso cuando el DANE lo suprime. La fila no se borra: hay direcciones guardadas que la apuntan';

-- Para poblar el selector de municipios de un departamento, que es la unica
-- consulta que esta tabla recibe.
CREATE INDEX idx_municipalities_by_department
    ON municipalities (department_code, name);
...
INSERT INTO municipalities (code, department_code, name) VALUES
    ('05001', '05', 'Medellín'),
    ('05002', '05', 'Abejorral'),
    ('05004', '05', 'Abriaquí'),
    ('05021', '05', 'Alejandría'),

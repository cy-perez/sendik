 ...
 *
 * <p><strong>Cambia esa y no crea una segunda.</strong> El identificador, la cuenta duena,
 * la fecha de creacion y la marca de predeterminada sobreviven: lo unico que se reescribe
 * son los siete campos y la fecha de actualizacion.
 *
 ...
 * ese identificador existe y que es de alguien. Como una direccion no la ve nadie mas que
 * su duena (RN-098), ni siquiera hay que admitir que existe.
 */
public class EditShippingAddressUseCase {
    ...
    public ShippingAddressView execute(EditShippingAddressCommand comando) {
        ...
        ShippingAddress editada = ShippingAddressFactory.editada(actual, comando.datos(), reloj.instant());

        ShippingAddressFactory.exigirMunicipioVigente(division, editada.municipio());

        direcciones.guardar(editada);

        return vistas.de(editada);
    }
}

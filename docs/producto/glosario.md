...
| Línea del carrito | `CartLine` | Un ítem del carrito **cruzado con su publicación en el momento de leer**. De ahí salen las dos cosas que el ítem guardado no sabe: si todavía está disponible y si el precio cambió desde que entró. Son dos conceptos y no uno: `CartItem` es lo que está guardado —su identidad es el par persona-producto y sobrevive a que la publicación se venda—; `CartLine` es lo que se ve, y se arma y se tira en cada lectura. |
...
| Dirección de entrega | `ShippingAddress` | A dónde, a nombre de quién y con qué teléfono llega lo que alguien compra. **Siempre con apellido y nunca «dirección» a secas**: en este proyecto esa palabra ya significa URL —el glosario la usa así en `SearchQuery` y en `RestrictedFileStore`, y ADR-0038 se llama «la dirección del cliente» hablando de una IP—. Guardarla no la comparte con nadie hasta que hay pago aprobado (RN-098). |
| Libreta de direcciones | `AddressBook` | Las direcciones de entrega guardadas por una persona, con exactamente una predeterminada mientras haya alguna (RN-099). Hasta diez (RN-101). |
| Dirección predeterminada | `DefaultAddress` | La que se propondrá al comprar. La primera lo es sin que se lo pidan, y al borrarla pasa a la más reciente de las que quedan. |
| División político-administrativa | `GeographicDivision` | La lista oficial del DANE de la que se elige a dónde va una dirección: 33 departamentos y 1122 municipios. Se dice así y no «división geográfica»; en el código el puerto lleva ese nombre. La ruta pública sí es `/api/v1/locations`, porque las rutas van en inglés y en plural y «political-administrative-division» no es un nombre de recurso: es la única divergencia y queda anotada aquí para que no se lea como descuido. Es dato de referencia sembrado y de solo lectura (ADR-0040). |
| Departamento | `Department` | Una de las 33 unidades del primer nivel de la división político-administrativa del DANE. No se escribe: se elige (RN-100). No se traduce, porque es un nombre propio. |
| Municipio | `Municipality` | Una de las 1122 del segundo nivel, que incluye las áreas no municipalizadas y la isla. Su código lleva dentro el del departamento, y de eso depende que la API no pida los dos. **Bogotá D.C. es departamento y municipio a la vez.** Cuando el DANE suprime uno, se marca inactivo y no se borra: hay direcciones guardadas que lo apuntan. |
| Dirección de origen | `OriginAddress` | Desde dónde despacha quien vende: municipio elegido del DANE, línea, complemento, indicaciones para la recogida y código postal. **Una por cuenta y sin identificador propio** (RN-105). Con apellido, como la de entrega, y por lo mismo. Sin nombre ni teléfono: son del remitente, que es el perfil (RN-106). Guardarla no promete recogida (RN-109). |
| Remitente | `Sender` | Quien entrega la mercancía al transportador y es parte del contrato de transporte. En Sendik es siempre **el vendedor**, aunque la guía la emita Sendik por su cuenta (RN-078): el titular de la cuenta, con el nombre y el teléfono de su perfil (RN-106). No es un dato que se escriba en la dirección de origen: se lee del perfil. En inglés jurídico es *shipper*; en el código se llama `Sender` en todas partes, y no las dos cosas. |
| Ciudad del perfil | `City` | Lo que se muestra como ciudad de una persona. Texto libre mientras no tiene dirección de origen; con origen, es su municipio con el departamento al lado y se cambia desde el origen (RN-107, ADR-0042). Clasificada como pública, aunque hoy ninguna respuesta pública la muestra. |
